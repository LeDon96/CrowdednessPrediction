...
def bubbleplot(dataset, x_column, y_column, bubble_column, z_column=None,
    ...
    if time_column:
        hours = dataset[time_column].unique()
    else:
        hours = None
        show_slider = False
        show_button = False
    ...
    if category_column:
        ...
        grid = make_grid_with_categories(dataset, column_names, time_column, category_column, hours, categories)
        ...
        col_name_template = '{}+{}_grid'
        grid = make_grid(dataset, column_names, time_column, hours)
        if show_legend is None:
            showlegend = False
        else: 
            showlegend = show_legend
    ...
    if category_column:
        ...
        for category in categories:
            if time_column:
                hour = min(hours) # The earliest hour for the base frame
                col_name_template_hour = col_name_template.format(hour, {}, {})
            else:
                col_name_template_hour = '{}+{}_grid'
            trace = get_trace(grid, col_name_template_hour, x_column, y_column, 
        ...
        if time_column: # Only if time_column is not None
            for hour in hours:
                ...
                if show_slider:
                    add_slider_steps(sliders_dict, hour)
...
def make_grid(dataset, column_names, time_column, hours=None):
    ...
    if time_column:
        ...
        for hour in hours:
            dataset_by_hour = dataset[(dataset[time_column] == int(hour))]
            for col_name in column_names:
                # Each column name is unique
                temp = col_name_template.format(hour, col_name)
                if dataset_by_hour[col_name].size != 0:
                    grid = grid.append({'value': list(dataset_by_hour[col_name]), 'key': temp}, ignore_index=True)
    else:
...
def make_grid_with_categories(dataset, column_names, time_column, category_column, hours=None, categories=None):
    '''Makes the grid for the plot as a pandas DataFrame by-passing the use of plotly.grid_objs
    that is unavailable in the offline mode for plotly. The grids are designed using the `col_name_template`
    ...
    if time_column:
        ...
        for hour in hours:
            for category in categories:
                dataset_by_hour_and_cat = dataset[(dataset[time_column] == int(hour)) & (dataset[category_column] == category)]
                for col_name in column_names:
                    # Each column name is unique
                    temp = col_name_template.format(hour, col_name, category)
                    if dataset_by_hour_and_cat[col_name].size != 0:
                        grid = grid.append({'value': list(dataset_by_hour_and_cat[col_name]), 'key': temp}, ignore_index=True) 
    else:
        col_name_template = '{}+{}_grid'
        for category in categories:
...
def add_slider_steps(sliders_dict, hour):
...
def get_trace(grid, col_name_template, x_column, y_column, bubble_column, z_column=None, size_column=None, 
    ...
    trace = {
        ...
        'text': "Crowdedness Count: " + str(grid.loc[grid['key']==col_name_template.format(size_column, category), 'value'].values[0]),
        'mode': 'markers'
        }
